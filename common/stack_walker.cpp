...
#define UnASLRAddr(addr) ((addr + 0x400000) - base_address_sw)
...
{
  DWORD base_address_sw = (DWORD)GetModuleHandle(nullptr);
  CONTEXT c;
...
#error "Platform not supported!"
  ...
  for (frameNum = 0; ; ++frameNum )
  {
    ...
    {
		this->OnDbgHelpErr("StackWalk64", GetLastError(), UnASLRAddr(s.AddrPC.Offset));
      break;
    }
    ...
    {
		this->OnDbgHelpErr("StackWalk64-Endless-Callstack!", 0, UnASLRAddr(s.AddrPC.Offset));
      break;
    }
    if (s.AddrPC.Offset != 0)
    {
      ...
      {
		  this->OnDbgHelpErr("SymGetSymFromAddr64", GetLastError(), UnASLRAddr(s.AddrPC.Offset));
      }
      ...
      { // yes, we have SymGetLineFromAddr64()
        ...
        {
			this->OnDbgHelpErr("SymGetLineFromAddr64", GetLastError(), UnASLRAddr(s.AddrPC.Offset));
        }
      } // yes, we have SymGetLineFromAddr64()
      ...
      {
		  this->OnDbgHelpErr("SymGetModuleInfo64", GetLastError(), UnASLRAddr(s.AddrPC.Offset));
      }
    } // we seem to have a valid PC
